...
-(void)tick:(double)delta;
{
    ...
    */
    ...
    if(move.x != 0.0f) {
        ...
        DTTraceResult *res = [self.world traceBox:self.size from:[Vector2 vectorWithX:self.position.x+offset y:self.position.y+0.5] to:[Vector2 vectorWithX:self.position.x+offset+move.x y:self.position.y+0.5] exclude:self ignoreEntities:YES inverted:YES];
        ...
        DTTraceResult *res = [self.world traceBox:self.size from:[Vector2 vectorWithX:self.position.x-0.5 y:self.position.y+offset] to:[Vector2 vectorWithX:self.position.x-0.5 y:self.position.y+move.y+offset] exclude:self ignoreEntities:YES inverted:YES];
