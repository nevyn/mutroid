...
- (BOOL)loadResource:(DTSound *)sound usingManager:(DTResourceManager *)manager error:(NSError *__autoreleasing *)error
{
    ...
    sound.sample = [dec decodeFileAtPath:soundUrl.path error:error];
    if(!sound.sample)
        return NO;
    return YES;
}
